...
use RendezVous qw(entete_standard affiche_ecran_rdv_print);
...
if(defined($parametres{ident_id})) {
  ...
  unless (@collaborateur) {
	print $cgi->header(), $cgi->start_html(title=>'Imprime rendez-vous'); 
...
#	die "Le parametre $id n'est pas défini dans la base dans la base de données";
  ...
  if(verif_tps_connexion() == 0) { #Délai d'attente dépassé
    print $cgi->header(),$cgi->start_html({-head =>@liens, -Title => 'Imprime rendez-vous',
