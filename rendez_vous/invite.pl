...
if(defined($parametres{ident_id})) {
  ...
  unless (@collaborateur) {
	print $cgi->header(), $cgi->start_html({Title=>'Invite des participants'}); 
...
#	die "Le parametre $id n'est pas défini dans la base dans la base de données";
  ...
  if(verif_tps_connexion() == 0) { #Délai d'attente dépassé
    print $cgi->header(),$cgi->start_html({-head =>@liens, -Title => 'Invite des participants',
...
print $cgi->header(), $cgi->start_html({Title=>'Invite des participants', -onLoad => 'window.setTimeout("history.back()", 5000);'});
print $cgi->img({-src=>"$rep/images/en_construction.png", -alt =>"En construction"}), $cgi->h1("La fonction \"Inviter les participants\" n'a pas encore été développée");
print $cgi->em("(Vous allez être redirigé vers l'écran précédent dans 5 secondes)");
print $cgi->end_html();
