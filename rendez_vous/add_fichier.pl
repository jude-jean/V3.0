...
if(defined($parametres{ident_id})) {
  ...
  unless (@collaborateur) {
	print $cgi->header(), $cgi->start_html({Title=>'Insère un fichier'}); 
...
#	die "Le parametre $id n'est pas défini dans la base dans la base de données";
  ...
  if(verif_tps_connexion() == 0) { #Délai d'attente dépassé
    print $cgi->header(),$cgi->start_html({-head =>@liens, -Title => 'Insère un fichier',
...
print $cgi->header(), $cgi->start_html({Title=>'Insère un fichier', -onLoad => 'window.setTimeout("history.back()", 5000);'});
print $cgi->img({-src=>"$rep/images/en_construction.png", -alt =>"En construction"}), $cgi->h1("La fonction \"Insère un fichier\" n'a pas encore été développée");
