...
var mois = [['Janvier', 'Janv', 0], ['Février', 'Fév', 1] ,['Mars', 'Mars', 2], ['Avril', 'Avr', 3], ['Mai', 'Mai', 4], ['Juin', 'Juin', 5], ['Juillet', 'Juil', 6], ['Août', 'Août', 7], ['Septembre', 'Sep', 8], ['Octobre', 'Oct', 9], ['Novembre', 'Nov', 10], ['Décembre', 'Déc', 11]];
var posRdvsY;
...
function ma_gestion_erreur(msg, url, line) {
  ecrire_dans_console(' Une erreur est survenue dans le code Javscript, voici les détails :\n Message d\'erreur : '+msg+'\n Ligne N° : '+line);
  return true;
}
...
// Calcule la distance entre le rdv et le point ou l'événement a eu lieu
...
// On enregistre les gestionnaires d'événenments
	document.addEventListener("mousemove", moveHandler, true);
	document.addEventListener("mouseup", upHandler, true);
// !l'�v�nement est trait�, on bloque sa propagation
...
// R�cup�ration de l'ident du rdv
...
// Désenregistre les gestionnaires d'événements
...
// Il faut maintenant modifier les champs cachés debut et fin qui seront utilisés pour constituer la chaine update_rdv
...
// Dans l'avenir, il faudra tenir compte peut-�tre du type d'affichage en particulier des minutes. Aujourd'hui, ce n'est pas le cas				
...
// Calcule la distance entre le rdv et le point ou l'�v�nement a eu lieu
	...
	ecrire_dans_console ("event.clientY = "+event.clientY+", top rdv = "+(parseInt(rdv.style.top))+", top rdv calcul� = "+top_rdv+", hauteur rdv = "+hauteur_rdv);
...
//  ecrire_dans_console("Le nom du bouton ayant �t� valid� est : "+bouton.value+"\nLa valeur des arguments sont :\n-.Etat = "+args.etat+"\nIdent = "+args.ident);
  switch (bouton.value) {
    case 'Calendrier' :
//      ecrire_dans_console("L'utilisateur a appuy� sur le bouton <Calendrier>");
      req = location.search;
...
//      ecrire_dans_console("La nouvelle valeur de req = "+req);
    ...
    case "Rapports d'activités" :
    case "Rapports d'activites" :
//      ecrire_dans_console("L'utilisateur a appuy� sur le bouton <Rapports d'activit�s>");
      req = location.search;
      if(document.forms[1].elements['ident_id'].value != args['ident_id']) {
        req = req.replace(/ident_id=.*/, 'ident_id='+document.forms[1].elements['ident_id'].value);
      }
...
//      ecrire_dans_console("La nouvelle valeur de l'url est :"+location);
    ...
    case 'Compte' :
//      ecrire_dans_console("L'utilisateur a appuy� sur le bouton <Compte> et mois � pour valeur ");
      req = location.search;
      if(document.forms[1].elements['ident_id'].value != args['ident_id']) {
        req = req.replace(/ident_id=.*/, 'ident_id='+document.forms[1].elements['ident_id'].value);
      }
...
//      ecrire_dans_console("La nouvelle valeur de l'url est :"+location);
    ...
    case "Données sociales" :
//      ecrire_dans_console("L'utilisateur a appuy� sur le bouton <Donn�es sociales>");
      req = location.search;
      if(document.forms[1].elements['ident_id'].value != args['ident_id']) {
        req = req.replace(/ident_id=.*/, 'ident_id='+document.forms[1].elements['ident_id'].value);
      }
...
//  ecrire_dans_console("Le titre de la fenetre est : "+titre[0].firstChild.data);
  ...
  if(args.mois == null) {
//    ecrire_dans_console("Le param�tre mois dans la ligne de commande est nul");
    args.mois = document.forms[1].elements['mois'].value;
  }
/*  else {
    ecrire_dans_console("Le param�tre mois dans la ligne de commande est �gale � :"+args.mois);
  }*/
  if(args.jour == null) {
    args.jour = document.forms[1].elements['jour'].value;
  }
  nb_rdv = document.forms[1].elements['nb_rdv'].value;
//  ecrire_dans_console('La valeur du champs ann�e est :'+args.annee+' la valeur du champs mois est :'+args.mois+'\nLa valeur du champs jour est :'+args.jour);
...
//          list_rdv_apres = document.forms[1].elements['apres_'+id_rdv].value.split(/\s+/);
//          ecrire_dans_console('Liste des rdv apr�s = '+list_rdv_apres);
          largeur = largeur_totale*taille;
          rdv.style.left =  ajust_gauche + (decalage*largeur_totale)+'px';
//          ecrire_dans_console('La largeur de l\'�l�ment '+rdv.id+' est : '+largeur+'px, son d�calage est de : '+decalage+'.\nSon parent a pour la largeur :'+parent_rdv.offsetWidth);
...
// Pour assurer une parfaite portabilit�, il faudrait faire un test pour sur le type du browser
...
/*  if(elt.value.search(/^[a-z][a-z0-9]{3,}/) == -1) {
  ...
  if((elt.value.length <= 3) ||
     ...
     (elt.value.search(/[\$\^\/\\=\+\#\{\}\"\(\)\[\]\|\`\!\?\,\;\:]/) >= 0)) {
        ecrire_dans_console('Le login ne peut :\n -être vide ou avoir une taille inférieure à 3\n -avoir une taille supérieure à 20\n -commencer par un chiffre ou par un point\n -être une combinaison de blancs et de chiffres\n -comprendre des caractères tels que :\n   $, ^, \\, /, +, =, \(, \), [, ], {, }, \", #, |,\`, !,?, ,, ;, : etc. ');
        elt.focus();
        return false;
  }
...
//  ecrire_dans_console('Sous menu : '+smenu+' le bouton enfonc� est : '+bouton.name+' et sa valeur est '+bouton.value+' sa forme contient '+bouton.form.length+' �l�ments');
  try {
/*    var msg = '';
    for(var i = 0; i < bouton.form.length; i++) {
       msg += ' El�ment n� '+i+' : '+bouton.form.elements[i].name+ ' sa valeur : '+bouton.form.elements[i].value+'\n';
    }
    ecrire_dans_console('La liste des �l�ments du formulaire\n'+msg);
*/
    if(smenu == 'identification') {
      if(bouton.value == 'OK') {
        if((bouton.form.elements['nom'].value != bouton.form.elements['nom_old'].value) ||
          ...
          ecrire_dans_console('Vous avez modifié certains champs dans le formulaire, vous ne pouvez utiliser le bouton OK :\n 1) -Soit vous désirez sauvegarder vos modifications et quitter le menu, cliquer successivement sur les boutons Appliquer et OK\n 2) -Soit vous souhaitez quitter le menu sans sauvegarder vos modifications, cliquer successivement sur les boutons Rétablir puis OK');
          return false;
        }
        return true;
      }
      else if(bouton.value == 'Appliquer') { // Réalisation des tests
        if(bouton.form.elements['nom'].value != bouton.form.elements['nom_old'].value) {
        ...
        if(bouton.form.elements['prenom'].value != bouton.form.elements['prenom_old'].value) {
          if((bouton.form.elements['prenom'].value.length == 0) ||
             ...
             (bouton.form.elements['prenom'].value.search(/[\$\^\/\\=\+\#\{\}\"\(\)\[\]\|\`\!\?\,\;\:]/) >= 0)) {
                ecrire_dans_console('Le prénom ne peut :\n -être vide\n -avoir une taille supérieure à 20\n -commencer par un chiffre, un espace ou par un point\n -être une combinaison de blancs et de chiffres\n -comprendre des caractères tels que :\n   $, ^, \\, /, +, =, \(, \), [, ], {, }, \", #, |,\`, !,?, ,, ;, : etc. ');
        ...
        if(bouton.form.elements['login'].value != bouton.form.elements['login_old'].value) {
          if((bouton.form.elements['login'].value.length <= 3) ||
             ...
             (bouton.form.elements['login'].value.search(/[\$\^\/\\=\+\#\{\}\"\(\)\[\]\|\`\!\?\,\;\:]/) >= 0)) {
                ecrire_dans_console('Le login ne peut :\n -être vide ou avoir une taille inférieure à 4\n -avoir une taille supérieure à 20\n -commencer par un chiffre, un espace ou par un point\n -être une combinaison de blancs et de chiffres\n -comprendre des caractères tels que :\n   $, ^, \\, /, +, =, \(, \), [, ], {, }, \", #, |,\`, !,?, ,, ;, : etc. ');
        ...
        if(bouton.form.elements['tel_perso'].value != bouton.form.elements['tel_perso_old'].value) {
          if(!((bouton.form.elements['tel_perso'].value.length >0) &&
               ...
               (bouton.form.elements['tel_perso'].value.search(/^\d{10}$/) == 0))) {
                  ...
                  ecrire_dans_console('Le champ Tél perso est incorrect.\nIl doit obligatoirement comporter un nombre de 10 chiffres');
                  return false;
          }
        }
        return true;
          ...
          if(bouton.value == 'Appliquer') {
            if((bouton.form.elements['pswd_new1'].value != bouton.form.elements['pswd_new2'].value) ||
               ...
               (bouton.form.elements['pswd_new1'].value.length <= 3)) {
                 ecrire_dans_console('Erreur avec le nouveau mot de passe : \n -Soit le deuxième mot de passe saisi ne correspond pas au premier\n -Soit sa taile est inférieure 4');
                 return false;
               }
            return true;
          }
...
//  ecrire_dans_console(msg);
  if(((ret1 = elt.name.search(/^pmatin_/))== 0) || ((ret2 = elt.name.search(/^paprem_/))== 0)) {
    switch(valeur) {
      case ' ':
      case '0' :
        // ecrire_dans_console('Le compteur a modifié est nb_dispo. Sa valeur est'+elt.form.elements['nb_dispo'].value);
        v = Number(elt.form.elements['nb_dispo'].value);
        v += 0.5;
        // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        elt.form.elements['nb_dispo'].value = v;// à cause de l'arrondi
        break;
     case '1' :
     case '2' :
        // ecrire_dans_console('Le compteur a modifié est nb_presence. Sa valeur est'+elt.form.elements['nb_presence'].value);
        v = Number(elt.form.elements['nb_presence'].value);
        v += 0.5;
        // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        elt.form.elements['nb_presence'].value = v;// à cause de l'arrondi
       break;
     case '3':
        // ecrire_dans_console('Le compteur a modifié est nb_cp. Sa valeur est'+elt.form.elements['nb_cp'].value);
        v = Number(elt.form.elements['nb_cp'].value);
        v += 0.5;
        // // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        elt.form.elements['nb_cp'].value = v;// à cause de l'arrondi
        v = Number(elt.form.elements['nb_absence'].value);
        v+= 0.5;
        elt.form.elements['nb_absence'].value = v;
       break;
      case '4':
        // ecrire_dans_console('Le compteur a modifié est nb_rtt. Sa valeur est'+elt.form.elements['nb_rtt'].value);
        v = Number(elt.form.elements['nb_rtt'].value);
        v += 0.5;
        // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        elt.form.elements['nb_rtt'].value = v;// à cause de l'arrondi
        v = Number(elt.form.elements['nb_absence'].value);
        v+= 0.5;
        elt.form.elements['nb_absence'].value = v;
       break;
     case '5':
        // ecrire_dans_console('Le compteur a modifié est nb_maladie. Sa valeur est'+elt.form.elements['nb_maladie'].value);
        v = Number(elt.form.elements['nb_maladie'].value);
        v += 0.5;
        // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        elt.form.elements['nb_maladie'].value = v;// � cause de l'arrondi
        v = Number(elt.form.elements['nb_absence'].value);
        v+= 0.5;
        elt.form.elements['nb_absence'].value = v;
       break;
     case '6':
        // ecrire_dans_console('Le compteur a modifié est nb_recup. Sa valeur est'+elt.form.elements['nb_recup'].value);
        v = Number(elt.form.elements['nb_recup'].value);
        v += 0.5;
        // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        elt.form.elements['nb_recup'].value = v;// à cause de l'arrondi
        v = Number(elt.form.elements['nb_absence'].value);
        v+= 0.5;
        elt.form.elements['nb_absence'].value = v;
       break;
     case '7':
        // ecrire_dans_console('Le compteur a modifié est nb_formation. Sa valeur est'+elt.form.elements['nb_formation'].value);
        v = Number(elt.form.elements['nb_formation'].value);
        v += 0.5;
        // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        elt.form.elements['nb_formation'].value = v;// � cause de l'arrondi
        v = Number(elt.form.elements['nb_absence'].value);
        v+= 0.5;
        elt.form.elements['nb_absence'].value = v;
       break;
     case '8':
        // ecrire_dans_console('Le compteur a modifié est nb_excep. Sa valeur est'+elt.form.elements['nb_excep'].value);
        v = Number(elt.form.elements['nb_excep'].value);
        v += 0.5;
        elt.form.elements['nb_excep'].value = v;// � cause de l'arrondi
        v = Number(elt.form.elements['nb_absence'].value);
        v+= 0.5;
        elt.form.elements['nb_absence'].value = v;
       break;
     case '9':
        // ecrire_dans_console('Le compteur a modifié est nb_sssolde. Sa valeur est'+elt.form.elements['nb_sssolde'].value);
        v = Number(elt.form.elements['nb_sssolde'].value);
        v += 0.5;
        elt.form.elements['nb_sssolde'].value = v;// � cause de l'arrondi
        v = Number(elt.form.elements['nb_absence'].value);
        v+= 0.5;
        elt.form.elements['nb_absence'].value = v;
       break;
...
//  ecrire_dans_console('Le tableau option a '+elt.options.length+' éléments.\Son dernier élément a pour texte : '+elt.options[elt.options.length - 1].text+'\nLe tableau data_save a '+data_save.length+' éléments.\nValeur_pred est égale à '+valeur_pred);
  if(data_save.length == 0) {
    ...
    if(data_save[0].name != elt.name) {
      ecrire_dans_console('Votre demande d\'ajout d\'une nouvelle option dans la liste des choix pour l\'élément '+elt.name+' ne peut aboutir.\nIl y a déja une demande du même type en cours et non complétée pour l\'élément '+data_save[0].name+'\n\nVous devez terminer cette action avant d\'envisager d\'en ajouter pour l\'élément '+elt.name+'.\nVous pouvez :\n-Soit choisir une option et cliquer sur le bouton \'OK\'.\n-Soit fermer la fenêtre en cliquant sur le bouton \'Fermer\'.');
...
function recup_args() {
  ...
  for(var i = 0; i < paires.length; i++) {
    ...
    try {
      ...
      ecrire_dans_console('unescape(): Problème de codage avec la valeur de '+nom+' égale à '+valeur);
      try {
        args[nom] = decodeURIComponent(valeur);
      }
      catch(e) {
        args[nom] = valeur;
        ecrire_dans_console('decodeURIComponent(): Problème de codage avec la valeur de '+nom+' égale à '+args[nom]);
      }
      finally {
        ecrire_dans_console('Le décodage pour la valeur de '+nom+ ' est égale à '+args[nom]);
      }
    }
  }
  return args;
...
//  ecrire_dans_console('Taille de data_save apr�s la fonction : '+data_save.length);
}


function bascule_astreinte(bouton) { // ce sontdes cases � cocher
...
function visualiser_ra() {
  var rep = confirm(encode('Veillez à enregistrer vos données nouvellement saisies car elles seront irrémédiablement perdues.\n\tSouhaitez-vous continuer la visualisation du rapport d\'activités ?'));
  return rep;
}

function supprimer_ra() {
  var rep = confirm('La suppression d\'un rapport d\'activités entraine la perte de toutes les données saisies dans celui-ci.\n\tSouhaitez-vous supprimer ce rapport d\'activités ?');
  return rep;
}
...
function fermer_fenetre(opt) {
  if(opt == 1) {
    var rep = confirm('Si vous fermez cette fenêtre sans avoir sauvegardé les données,\nvous risquez de perdre les modifications effectuées.\n\nVoulez-vous fermer cette fenêtre ?');
    if(rep == true) {
      close();
    }
  }
...
function imprimer_ra(opt) {
  if(opt == 1) {
    var rep = confirm('Attention : Assurez-vous que vous avez sauvegardées les modifications réalisées.\nDans le cas contraire, elles seront irrémédiablement perdues.\n\nVoulez-vous continuer l\'impression?');
    if(rep == true) {
	  return true;
    }
    else {
...
//  AfficheNomsProprietes(selection.options[selection.selectedIndex]);
    ...
    switch(sel.value) {
      case ' ':
      case '0':
        // ecrire_dans_console('Le compteur a modifié est nb_dispo. Sa valeur est'+pres['nb_dispo'].value);
        v = Number(pres['nb_dispo'].value);
        v -= cpte;
        // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        pres['nb_dispo'].value = v;// � cause de l'arrondi
        break;
     case '1' :
     case '2' :
        // ecrire_dans_console('Le compteur a modifié est nb_presence. Sa valeur est'+pres['nb_presence'].value);
        v = Number(pres['nb_presence'].value);
        v -= cpte;
        // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        pres['nb_presence'].value = v;// � cause de l'arrondi
       break;
     case '3':
        // ecrire_dans_console('Le compteur a modifié est nb_cp. Sa valeur est'+pres['nb_cp'].value);
        v = Number(pres['nb_cp'].value);
        v -= cpte;
        // // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        pres['nb_cp'].value = v;// � cause de l'arrondi
       break;
      case '4':
        // ecrire_dans_console('Le compteur a modifié est nb_rtt. Sa valeur est'+pres['nb_rtt'].value);
        v = Number(pres['nb_rtt'].value);
        v -= cpte;
        // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        pres['nb_rtt'].value = v;// � cause de l'arrondi
       break;
     case '5':
        // ecrire_dans_console('Le compteur a modifié est nb_maladie. Sa valeur est'+pres['nb_maladie'].value);
        v = Number(pres['nb_maladie'].value);
        v -= cpte;
        // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        pres['nb_maladie'].value = v;// � cause de l'arrondi
       break;
     case '6':
        // ecrire_dans_console('Le compteur a modifié est nb_recup. Sa valeur est'+pres['nb_recup'].value);
        v = Number(pres['nb_recup'].value);
        v -= cpte;
        // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        pres['nb_recup'].value = v;// � cause de l'arrondi
       break;
     case '7':
        // ecrire_dans_console('Le compteur a modifié est nb_formation. Sa valeur est'+pres['nb_formation'].value);
        v = Number(pres['nb_formation'].value);
        v -= cpte;
        // ecrire_dans_console('La nouvelle valeur de v est : '+v);
        pres['nb_formation'].value = v;// � cause de l'arrondi
       break;
     case '8':
        // ecrire_dans_console('Le compteur a modifié est nb_excep. Sa valeur est'+pres['nb_excep'].value);
        v = Number(pres['nb_excep'].value);
        v -= cpte;
        pres['nb_excep'].value = v;// � cause de l'arrondi
       break;
     case '9':
        // ecrire_dans_console('Le compteur a modifié est nb_sssolde. Sa valeur est'+pres['nb_sssolde'].value);
        v = Number(pres['nb_sssolde'].value);
        v -= cpte;
        pres['nb_sssolde'].value = v;// � cause de l'arrondi
       break;
    }
...
//  var sel = bouton.form.elements[selection.toString()];
  ...
  if(sel_id == 0) {// tous les champs sont s�lectionn�s
    for (var i = 1; i <= nb_jours; i++) {
       pres_matin = 'pmatin_'+i;
       pres_aprem = 'paprem_'+i;
...
//    ecrire_dans_console('Le nodeName de racine est : '+racine.nodeName+' son Id est : '+racine.id+' Son nobre d\'enfants est égale à : '+racine.childNodes.length);
    if(args.action == 'edition') {// Modification de la ligne de 'création'
  ...
  for(var i = 0; i < enfants.length; i++) {
    if(enfants[i].nodeName == 'A') {
      if(enfants[i].firstChild.nodeType == Node.TEXT_NODE) {
        if((enfants[i].title == 'Créer') && (enfants[i].target == 'Creation')&& (enfants[i].firstChild.data == args.maj)) {
//          ecrire_dans_console('indice = '+i+' : Le nodeName est '+enfants[i].nodeName+' et son url est '+enfants[i].href+'\n La class de son parent est '+enfants[i].parentNode.className+' son title est '+enfants[i].title+' Sa target est '+enfants[i].target+' Son texte est '+enfants[i].firstChild.nodeValue);
          ...
          ligne_creation = lg;
          nb_maj++;
//          ecrire_dans_console('indice = '+i+' : Le nodeName est '+enfants[i].nodeName+' et son url modifié est '+enfants[i].href+'\n La class de son parent est '+enfants[i].parentNode.className+' son title est '+enfants[i].title+' Sa target est '+enfants[i].target+' Son texte est '+enfants[i].firstChild.nodeValue);
          return;
        }
//       else {
...
//          ecrire_dans_console('indice = '+i+' : Le nodeName est '+enfants[i].nodeName+' et son url est '+enfants[i].href+'\n La class de son parent est '+enfants[i].parentNode.className+' son title est '+enfants[i].title+' Sa target est '+enfants[i].target+'\nALT de son enfant : '+enfants[i].firstChild.alt);
          if((enfants[i].firstChild.title == 'Créer') && (enfants[i].target == 'Creation')&& (enfants[i].firstChild.alt == ('Créer-'+args.maj))) {
            enfants[i].parentNode.replaceChild(frag, enfants[i]);
            nb_maj++;
            return;
          }
//          else {
//            ecrire_dans_console('Les valeurs du test sont : title : '+enfants[i].firstChild.title+', target : '+enfants[i].target+', Alt : '+enfants[i].firstChild.alt+' test� avec la valeur \'Cr�er-'+args.maj+'\'');
//          }
        }
      }
    }
...
//        ecrire_dans_console('Lancement r�cursif de traite_ra_ligne3col');
        traite_ra_ligne3col(enfants[i], lg, args, elt_a, frag, text_edit);
//        ecrire_dans_console('Fin du lancement r�cursif de traite_ra_ligne3col');
      }
    }
  }
}
...
function AfficheNomsProprietes(obj) {
  ...
  ecrire_dans_console("les propriétés de l'objet "+obj.name+" sont :\n"+noms);
