...
#    $id = undef;
# }
  if($connexion eq 'D&eacute;connexion') {
    deconnexion();
  ...
  if($ecran_actuel eq "Rapports d'activités") {
    print $cgi->redirect("$rep_pl/rapports_activites/show.pl?ident_id=$id&ident_user=$collaborateur[3]");
	exit;
  }	
  if($ecran_actuel eq "Calendrier") {
    print $cgi->redirect("$rep_pl/calendrier/show.pl?ident_id=$id&ident_user=$collaborateur[3]");
	exit;
  }	
...
sub entete_standard {
  ...
  if($ecran_actuel eq 'Calendrier') {
    ...
    print $cgi->start_html({-head => @liens, -Title => "etechnoserv.com v3.0", -script => \%script,
              -encoding => 'UTF-8', -style =>\%style, -xbase => "$url", onLoad => 'return addCharsetUtf8InForm();'});
  }
...
# Affichage dans un div gauche et dans un div droite => 2 colonnes
  ...
  print $cgi->start_div({-id=> 'infos_generales'}), "\n",
        $cgi->span("Informations g&eacute;n&eacute;rales >> Date d'arriv&eacute;e : $date[2]\/$date[1]\/$date[0],",
        "Temps de pr&eacute;sence :",($Da >0)?"$Da an(s) ":'',($Dm >0)?"$Dm mois ":'',($Dj>0)?"$Dj jour(s)":'', $cgi->br, "Planning pr&eacute;visionnel :");

  print $cgi->start_div({-id=>'col_gauche'}), "Edition Rapport d'Activit&eacute;s :",
     $cgi->br, "Prochaine formation :",
     $cgi->br, "Convocation visite m&eacute;dicale :",
     $cgi->br, "Convocation entretien d'&eacute;valuation :",
     $cgi->br, "N&eacute;gociation salariale :",
     $cgi->end_div(), "\n";
...
sub affiche_conges_payes {
  ...
  print $cgi->div({-class => 'Titre'}, 'D&eacute;compte des cong&eacute;s pay&eacute;s');
  print $cgi->start_div({-class => 'Libelle'});
  print $cgi->br, "Ann&eacute;e $annee ",
        $cgi->br, 'Ann&eacute;es ant&eacute;rieures ',
        $cgi->br, 'Total ' ;
...
sub affiche_rtt {
  ...
  print $cgi->div({-class => 'Titre'}, 'D&eacute;compte des RTT');
  print $cgi->start_div({-class => 'Libelle'});
  print $cgi->br, "Ann&eacute;e $annee ",
        $cgi->br, "Ann&eacute;e ",$annee - 1,
        $cgi->br, 'Total ' ;
  print "\n", $cgi->end_div(); # Fin du div libelle
  print $cgi->start_div({-class => 'Acquis'}), $cgi->span({-class => 'L1'},'Acquis');
  print $cgi->br, "$acquis_now",
...
sub menu_compte_communication {
  print $cgi->start_div({-id => 'Communication'}),
   $cgi->start_fieldset(), $cgi->legend('Donn&eacute;es chez le client'), $cgi->start_div({-class => 'infos'}), $cgi->start_div({-class => 'Ligne2col'}), $cgi->div({-class => 'Ligne2col1'},"E-mail mission"),
  ...
  print $cgi->start_fieldset(), $cgi->legend('Donn&eacute;es personnelles'), $cgi->start_div({-class => 'infos'}), $cgi->start_div({-class => 'Ligne2col', -id => 'derniere_ligne'}), $cgi->div({-class => 'Ligne2col1'}, "T�l. perso"),
...
sub menu_compte_missions {
  ...
  if(defined($client)) {
    ...
    print $cgi->p($cgi->em("Aucun client n'est affect&eacute;"), $cgi->br, "En cas d'erreur, contacter votre responsable de contact");
  }
...
# Les contr�les sur les donn�es avant l'enregistrement
    ...
    $msg_maj[0] = $smenu;
    $msg_maj[1] = 1;
    $msg_maj[2] = "Mise à jour effectu&eacute;e";
    return;
  }
  if($smenu eq 'communication') {
...
# Les contr�les sur les donn�es avant l'enregistrement
    if(menu_compte_verif_donnees($smenu, $pswd, $pswd1, $pswd2) == 0) {
      ...
      $msg_maj[2] = "Mise à jour du mot de passe effectu&eacute;e";
      return;
    }
  }
...
sub menu_compte_verif_donnees {
    ...
    unless($nom eq $collaborateur[1]) {
      if((length $nom <= 0) || (length $nom > 20) || ($nom =~/^[0-9\.\s]/) ||
         ...
         ($nom =~ /[\$\^\/\\=\+\#\{\}\"\(\)\[\]\|\`\!\?\,\;\:]/)) {
            $msg_maj[0] = $smenu;
            $msg_maj[1] = 1;
            $msg_maj[2] = "Le champ nom ne peut être vide, avoir une taille sup&eacute;rieure à 20, commencer par un chiffre, un espace ou par un point. Il ne peut être une combinaison de blancs et de chiffres et comprendre des caractères tels que :".$cgi->br." &nbsp; &nbsp;$, ^, \\, /, +, =, \(, \), [, ], {, }, \", #, |,\`, !,?, ,, ;, : etc. ";
            return 1;
      }
    }
    unless($prenom eq $collaborateur[2]) {
      if((length $prenom <= 0) || (length $prenom > 20) || ($prenom =~/^[0-9\.\s]/) ||
         ...
         ($prenom =~ /[\$\^\/\\=\+\#\{\}\"\(\)\[\]\|\`\!\?\,\;\:]/)) {
            $msg_maj[0] = $smenu;
            $msg_maj[1] = 2;
            $msg_maj[2] = "Le champ prenom ne peut être vide, avoir une taille sup&eacute;rieure à 20, commencer par un chiffre, un espace ou par un point. Il ne peut être une combinaison de blancs et de chiffres et comprendre des caractères tels que :".$cgi->br." &nbsp; &nbsp;$, ^, \\, /, +, =, \(, \), [, ], {, }, \", #, |,\`, !,?, ,, ;, : etc. ";
            return 2;
      }
    }
    unless($login eq $collaborateur[3]) {
      if((length $login < 4) || (length $login > 20) || ($login =~/^[0-9\.\s]/) ||
         ...
         ($login =~ /[\$\^\/\\=\+\#\{\}\"\(\)\[\]\|\`\!\?\,\;\:]/)) {
            $msg_maj[0] = $smenu;
            $msg_maj[1] = 3;
            $msg_maj[2] = "Le champ login ne peut être vide, avoir une taille inf&eacute;rieure à 4 et sup&eacute;rieure à 20, commencer par un chiffre, un espace ou par un point. Il ne peut une combinaison de blancs et de chiffres et comprendre des caractères tels que :".$cgi->br." &nbsp; &nbsp;$, ^, \\, /, +, =, \(, \), [, ], {, }, \", #, |,\`, !,?, ,, ;, : etc. ";
            return 3;
      }
    }
    return 0;
    ...
    unless($tel_mission eq $collaborateur[11]) {
      if(!((length $tel_mission >0) && (length $tel_mission < 11) &&
          ($tel_mission =~ /^\d{10}$/))) {
            $msg_maj[0] = $smenu;
            $msg_maj[1] = 2;
            $msg_maj[2] = "Le champ T&eacute;l mission est incorrect. Il doit être un nombre de 10 chiffres";
            return 2;
      }
    }
    unless($tel_perso eq $collaborateur[12]) {
      if(!((length $tel_perso >0) && (length $tel_perso < 11) &&
          ($tel_perso =~ /^\d{10}$/))) {
            $msg_maj[0] = $smenu;
            $msg_maj[1] = 3;
            $msg_maj[2] = "Le champ T&eacute;l perso est incorrect. Il doit être un nombre de 10 chiffres";
            return 3;
      }
    }
    return 0;
  ...
  if($smenu eq 'mot_de_passe') {
    ...
    unless($pswd_md5 eq $collaborateur[4]) {
      $msg_maj[0] = $smenu;
      $msg_maj[1] = 1;
      $msg_maj[2] = "Le mot de passe saisi n'est pas &eacute;gal au mot de passe actuel";
      return 1;
    }
    ...
    if((length $pswd1 <= 3) || (length $pswd2 <= 3)) {
      $msg_maj[0] = $smenu;
      $msg_maj[1] = 1;
      $msg_maj[2] = "Erreur sur le nouveau mot de passe : Sa taille doit être au moins &eacute;gale à 4";
      return 1;
    }
    return 0;
  }
