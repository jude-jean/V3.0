...
if($ecran_actuel eq 'Appliquer') {
	if(compte_identification_enregistre_data() != 2) {;
		print $cgi->redirect($cgi->script_name."?no_msg=$no_msg&ident_id=$id&ident_user=".$cgi->param('login'));
		exit;
	}
	else {
		entete_standard();
...
sub compte_identification_enregistre_data {
#Enregistre les données du menu compte/identification et revient à la saisie
...
#    $msg_maj[0] = $smenu;
#    $msg_maj[1] = 1;
#    $msg_maj[2] = "Mise à jour effectuée";
#    return;
	}
	else {
# Utile car la variable $no_msg est alors mis à jour dans menu_compte_verif_donnees()	
		return 1;
	}
  }
}
