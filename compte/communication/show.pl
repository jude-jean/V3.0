...
if($ecran_actuel eq 'Appliquer') {
	if(compte_communication_enregistre_data() != 2) {
		print $cgi->redirect($cgi->script_name."?smenu=communication&no_msg=$no_msg&ident_id=$id&ident_user=".$cgi->param('login'));
		exit;
	}
	else {
		entete_standard();
...
sub compte_communication_enregistre_data {
